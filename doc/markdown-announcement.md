...
- doctree に Markdown ツリー `manual/{api,doc,capi}` を追加済み（1245 ファイル）
...
### フェーズ 4（将来）: 残タスクと新パイプライン

- bitclust gem のリリース（refe2 などローカルツール向けの Markdown 対応版）
- 旧バージョン（Ruby 3.0 より前）のドキュメントのサルベージ
  （凍結 `refm/` から同じ変換器を別スコープで再実行して回収）
- サルベージ完了後に旧 `refm/` ツリーを削除
