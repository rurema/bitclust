...
6（ブリッジ廃止）は**検証用にしばらく温存する方針に決定**（2026-07-07）:
markdown-findings.md のフォローアップと旧バージョン（< 3.0）サルベージが
完了してから削除する。それまで `BITCLUST_MD_BRIDGE=1`・`MarkdownBridge`・
`tools/md-bridge-check.rb` は維持。
