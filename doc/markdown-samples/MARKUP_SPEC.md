...
`# object main` のようにクラス名が定数形式でないオブジェクトページに限り
`self.` を使う。

特異メソッドのセクション配下では、将来的にプレフィクスなしの `### def name` を
ビルドエラーにする（キーワード/プレフィクスとセクションの不一致検出。
rurema/doctree#3291 のような分類間違いを機械検出するため。既存文書の
`ClassName.` 化が完了してから有効になる）。
