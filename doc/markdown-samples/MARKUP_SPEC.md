...
RD ベースの独自記法から Markdown ベースの記法へ移行するための仕様を定める。

## 背景と方針

- **ベース仕様**: GitHub Flavored Markdown (GFM)
- **プリプロセッサ**: 既存の指令を新 prefix `#%` で維持（Liquid には依存しない。旧 `#@` も後方互換で受理。§8）
...
|------|-------------------------------|--------|
| プリプロセッサ | Liquid (`{% if %}`) | `#%since`/`#%until`/`#%if` を維持（旧 `#@` も受理） |
| コメント | `{% comment %}` | `#%#` を維持（旧 `#@#` も受理） |
| include | Liquid `{% include %}` 予定 | `#%include` を維持（旧 `#@include` も受理） |
...
旧 RD 世界の `LIBRARIES` マニフェストと、ライブラリ→クラスの grouping 用 `#@include` は
...
- `since`/`until` は「エンティティ自体が版で出入りする」構造的ゲートを表す
  （旧 RD で `#@include` を `#@since`/`#@until` で囲っていたものに相当）。
...
- リスト内で使えるディレクティブは `#%since`/`#%until`/`#%version`
  （`V` / `A...B` / `A...` / `...B`）と、直前のゲートを反転する `#%else`、
  `#%end`、コメント行 `#%#`。入れ子は交差（since は max・until は min）。
  `#%if`（任意の条件式）は使えない
- 解釈できない `#%` 行・対応のない `#%else`/`#%end`・閉じていないゲート・
  インデントされた `#%` 行（列 0 でないと指令にならない）は
  ビルドエラー（`ParseError`）になる（無音でリストが打ち切られることはない）。
  `#` で始まる通常の YAML コメント行は無視される
- `#%version V` の版比較は `Gem::Version`（`3.4` と `3.4.0` は同じ版）。
  本文の `#%version V` が文字列一致なのとは異なる
...
- GFM に include 相当が無いため、共有手段として `#%include` を維持する（旧 `#@include` も受理）。
...
- `library`（§1.2.1 のゲート付きリスト）だけは前処理より前にファイル発見
  （MarkdownTree）が直接読むため、使えるディレクティブが
  `#%since`/`#%until`/`#%version`/`#%else`/`#%end`/`#%#` に限られる
  （`#%if` は不可）。
...
エントリキーワードはセクション（h2）の種別と一致していなければならない
（不一致はビルドエラー。例: `## Class Methods` 配下の `const`、`## Constants`
配下の `def`。キーワードはエントリの型決定には使われず型はセクションで決まる
ため、この検査がないと rurema/doctree#3291 のような分類間違いが出力に
現れないまま残る）。

RD との相互変換では `ClassName.method` をそのまま保持する（`self.` への変換はしない）。
クラスメソッドは `self.` プレフィクス形式（`### def self.new(...)`）でも書ける
（2026-08 対応。singleton method として解釈され、描画はどちらの形式でも
プレフィクスなしの `new(...)`）。ただし**推奨は `ClassName.` 形式**とする。
目的はセクション見出し（`## Class Methods` 等）に頼らず `def` の行だけを見て
メソッド種別が分かることなので、対象クラスまで明示される `ClassName.` を選ぶ。
`# object main` のようなオブジェクトページも同様にオブジェクト名を
プレフィクスにする（`### def main.include(*modules) -> self` など。
`main` は `fatal`・`ARGF.class` と同様に文法上クラス名として扱われる）。

特異メソッドのセクション配下では、プレフィクスなしの `### def name` は
ビルドエラーになる（プレフィクスとセクションの不一致検出。既存文書の
`ClassName.` 化（rurema/doctree#3393）と `main.` 化（rurema/doctree#3431）の
完了を受けて 2026-08 に有効化）。

### 3.2 RBS 形式（不採用）

RBS 型シグネチャをそのまま定義行に書く形式
（`### def self.try_convert: (untyped obj) -> Array[untyped]?` のような）は
**採用しない**。手書きのシグネチャは Ruby 構文形式（§3.1）に一本化する。

RBS の型情報はシグネチャ記法ではなく `bitclust rbssig` サブコマンドが提供する:
ruby/rbs のシグネチャを DB に取り込み、生成ページのメソッド定義行の直下に
RBS シグネチャ行として自動表示される。手書きする必要はない。

| 判定基準 | 扱い |
|---------|------|
| `def name(` または `def name ->` | Ruby 構文（§3.1） |
| `def name:` | RBS 形式とみなしビルドエラーで拒否 |

`def name:` の形を書いた場合はパース時に「unsupported method signature」として
明確なビルドエラーになる（誤った名前で DB に登録されたり statichtml が
クラッシュしたりしないよう、早期に拒否する）。

クラスメソッドを `self.` プレフィクスで書く形（`### def self.new(...)`）は
Ruby 構文形式として対応済み（§3.1。ただし推奨は `ClassName.`）。
...
RDの `@see` が複数行に分かれている場合は個別の項目になる:
...
RD の `[[type:target]]` から角括弧を1段減らし `[type:target]` とする。
...
モジュール関数の `?.` は RD の `.#` typemark に対応する（`?` は RBS の `self?` に由来）。
...
指令の prefix は `#%` を正式とする（bitclust#285、2026-07 決定）。
`#%` は CommonMark で見出し（ATX heading）として解釈されず
（ATX heading は `#` の後にスペースが必要）、かつ `@` を含まないため
GitHub 上で `#@since` の `@since` 部分がユーザー mention として
誤解釈される問題が起きない。

**後方互換**: 旧 prefix `#@` も引き続き受理される（凍結タグ
frozen-* の旧ソースを再取込する場合に必要。プリプロセッサは
`#@`/`#%` を同一に扱い、同一ファイル内の混在も許す）。
新規に書く文書では `#%` を使うこと。
...
`#%version A...B` は半開区間 [A, B) の省略記法
（`#%if (version >= "A" and version < "B")` と同義。bitclust#285）。
Ruby の終端排他 Range リテラルに合わせて **3点ドット**で書く
（`..` は終端を含むと誤解しやすいため受け付けない。終端を含む記法が
必要になった場合に `..` を割り当てられるよう温存している）。
`#%version A...`（A 以上 = `#%since A` と同義）、
`#%version ...B`（B 未満 = `#%until B` と同義）、
ドットなしの `#%version V`（その版のみ = `#%if (version == "V")` と同義）も書ける。
いずれも `#%else` と組み合わせられる。
...
RD と同じ挙動であり、**これを確定仕様とする**（2026-07 決定）。
...
- 制約: コードブロック内に**行頭リテラルの `#%`（旧 `#@` も同様）** は書けない
  （指令として処理される。manual/ は Ruby の文書でありこの記法自体を例示する
  需要は無い — 必要になった場合は行頭に空白を1つ入れる）
...
RD ではインデントされたテキストが整形済みテキスト（`<pre>`）になる。
...
RD の GNU 風引用 `` `token' ``（rd に code マークアップがなかった時代の
...
RD の定義リスト（`: term` / `  definition`）に相当する記法として、
...
地の文の `**strong**` は `<strong>` として描画される（bitclust#301）。
対応するのは保守的なサブセットのみ:

```markdown
このメソッドは**直接呼び出してはいけません**。
**`freeze` される**ので注意。
**[c:String]** も参照。
```

- 同一テキストノード内で閉じること（段落内の soft break は行結合されるためまたげる）
- 開き `**` の直後・閉じ `**` の直前は空白以外（`a ** b` は対象外）
- 内容に `*` を含まない（入れ子なし。`***both***` は対象外）
- 単語内には適用しない（開きの直前・閉じの直後が英数字・`_` なら対象外。
  `2**32` のような冪乗演算子を巻き込まないための規則で、
  和文の「これは**重要**です」は適用される）
- コードスパン内・コードブロック内は対象外（`` `**not strong**` `` はそのまま）
- `*em*` / `_em_` の斜体は多義性が高いため非対応
...
2. `### def ` → メソッドシグネチャ
   - `def ClassName.name`（または `def self.name`） → クラスメソッド / 特異メソッド
   - `def name` → インスタンスメソッド
   - `def name:`（RBS 形式）は不採用（§3.2）。ビルドエラーとして拒否される
3. `### const ` → 定数
4. `### gvar ` → グローバル変数
5. それ以外の `### ` → 本文小見出し
```
...
| `@see [[m:...]]` | `- **SEE** [m:...]` | リスト形式 |
| `#@since VER` | `#%since VER` | prefix を `#%` に変更（#285）・旧 `#@` も受理 |
| `#@until VER` | `#%until VER` | prefix を `#%` に変更（#285）・旧 `#@` も受理 |
| `#@if (cond)` | `#%if (cond)` | prefix を `#%` に変更（#285）・旧 `#@` も受理 |
| `#@else` | `#%else` | prefix を `#%` に変更（#285）・旧 `#@` も受理 |
| `#@end` | `#%end` | prefix を `#%` に変更（#285）・旧 `#@` も受理 |
| `#@include(file)` | `#%include(file)` | prefix を `#%` に変更（#285）・旧 `#@` も受理 |
| `#@#` | `#%#` | prefix を `#%` に変更（#285）・旧 `#@` も受理 |
...
各ファイルは `bin/rrd2md` による実際の RD→MD 変換出力から生成されたものである。
...
- [x] `[c:String]` 等のリンク記法（RD `[[...]]` ↔ MD `[...]` の双方向変換で解決）
...
- [x] `refe` コマンド等でのプレーンテキスト表示時の可読性確認（2026-07 確認済み。
  `refe`・`bitclust lookup` の表示は RD 時代から「バージョン解決済みソースを
  そのまま表示」であり、md ソースでも段落・リスト・参照・見出し・`` ```ruby ``
  コード例は RD ソースと同等以上の可読性。既知の劣化は
...
- [x] RBS 形式シグネチャの実運用テスト（2026-07 検証: 実装（パーサ・描画）が
  ...
  誤検知ゼロを検証済み）。その後、型情報は rbssig サブコマンドの埋め込み表示で
  提供する方針とし、記法としては不採用と確定（§3.2））
