...
> クラスリファレンスデータベースの構造の記述は現行実装とおおむね一致します
> （現在は `doc/` ディレクトリと `refs` ファイルも作られます）。
> 「C API データベース (予定)」の節は構想当時のもので、現在は
> FunctionDatabase として実装済みです。実際の構造は下記の予定とは異なり、
> `PREFIX/function/関数名` の 1 階層で、各ファイルはクラスリファレンス
> データベースと同じ「key=value 行＋空行＋ドキュメント」形式です。
